...
\subsection{\textbf{RQ1}}}\label{sec:eval:RQ1}
...
In table \ref{tab:feature_sfbr_table}, we display the average overlap degree of features for all projects.
...
\begin{table}[t]
\caption[Average Feature Overlap Degree of Projects]{Average feature overlap degree of all projects}
...
As discussed in chapter \ref{sec:research_questions}, a best practice for commits in feature development is that a commit should generally deal with only one responsibility in relation to features.
...
\subsection{\textbf{RQ2}}\label{sec:eval:RQ2}
...
The majority of \textsc{gzip}'s active commits are part of dataflow-based CFIs at $\math{37.1}$\%\footnote{this percentage is calculated only considering active commits of \textsc{gzip} and not its sub-module \textsc{gnulib}; we explained this choice more thouroughly in the Operationalization chapter}.
...
On the right-hand side of the columns in table \ref{tab:commit_exclusive_and_partial_dfbr_table}, we display the number and percentage of commits where only a subset of their interactions with features was discovered exclusively by our dataflow analysis.
These commits are most common in \textsc{gzip} with a count of $\math{46}$ making up almost fourth active commit at $\math{23.7}$\%.
...
\subsection{\textbf{RQ3}}\label{sec:eval:RQ3}
...
\subsection{\textbf{RQ1}}\label{sec:disc:RQ1}
...
In section \ref{sec:commit_concerns}, we explained that, due to feature overlap, the number of features a commit structurally interacts with can be a faulty estimate of how many responsibilities in relation to features a commit usually deals with.
To achieve a more accurate measure, we introduced the concept of feature related concerns of a commit (Definition \ref{def:commit_concerns}).
...
\subsection{\textbf{RQ2}}\label{sec:disc:RQ2}

We are able to confirm that structural CFIs heavily coincide data flow-based CFIs.
...
Furthermore, our dataflow analysis uncovers a significant amount of interactions between commits and features that we cannot find with our previous structural analysis.
...
While only $\math{11}$\% of active commits in \textsc{xz} affect features through dataflow, the majority of active commits do so in \textsc{gzip}.
...
For $\math{8}$, $\math{21}$ and $\math{44}$\% of active commits in \textsc{xz}, \textsc{bzip2} and \textsc{gzip}, it reveiled additional interactions with features that we could not discover with our structural analysis.
This means that the data of, on first sight unrelated, commits and features can be highly connected. 
Besides that, we found that many commits interacting with features both structurally and through dataflow, also interact with \emph{other} features \emph{only} through dataflow.
...
\subsection{\textbf{RQ3}}\label{sec:disc:RQ1}
