...
Nevertheless, we believe that our dataflow analysis is very important to fully understand commit-feature interactions and their frequency within a project.
...
When discussing the second part of RQ2, we will seek to underline this notion by determining how often commits affecting a feature through outside dataflow, structurally interact with other features. \\
...
This means that the majority of commits affecting a feature through are located outside of the feature they interact with.
Thus, most of the data belonging to the detected interactions of features originates outside of its regions and subsequently flows into them.
...
A high degree of feature-nesting can not only lead to many features sharing the same structurally interacting commits, but also the commits affecting them through outside dataflow.
...
Thus, we find that there also exists a lot of dataflow within the two clusters, where the code of the \textsf{no_time}-feature acts as a major intersection of said dataflow.
This is another product of \textsc{gzip}'s high degree of feature-nesting, where the features of the second cluster are nested inside different parts of the first cluster.
...
Although structural authors often interact with their respective features via outside dataflow, the average of slightly over 50\% across the mentioned projects is too low to establish a clear rule for them.
