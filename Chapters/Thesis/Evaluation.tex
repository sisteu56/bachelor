...
Around half of the author structurally interacting with a feature in \textsc{lrzip} also interact with it through outside dataflow.
...
This means that most of the data belonging to the detected interactions of features originates outside of its regions and subsequently flows into them.
...
One reason for this is that even features with little size are affected by many outside commits and only few inside commits.
...
During the evaluation of the first part of RQ2, we found that commits that structurally interact with features have an increased probability of influencing other features via outside dataflow.
...
All features except \textsf{force} of \textsc{gzip} have proportions of more than 0.2 and the majority of values range close to the average of a project.
...
This also means that commits not changing any feature-code are less likely to affect features through dataflow than our initial results would suggest.

- the regression plots of \textsc{gzip} show that its strong linear correlations are mainly caused by two clusters of features \\
...
- indeed, the majority of outside commits from the features of each cluster are identical \\
...
- thus, the two clusters are not formed by coincidence, but because the features of each cluster share much of their code amongst themselves \\
- at the same time, 25 inside commits of the first cluster make up more than 35\% of the commits affecting features of the second cluster through outside dataflow \\
...
- 47 of the commits affecting \textsf{no\_time} through outside dataflow are inside commits of \textsf{force} \\
- thus, we find that there does not only exist a lot of dataflow within the two clusters, but between the two clusters as well \\
- this is another product of the high degree of feature-nesting, where the features of the second cluster are nested inside different parts of the first cluster \\ 
\iffalse
- this observation in turn shows us the difficulties that a high degree of feature-nesting poses for the interpretability of our collected CFIs \\
- similarly to not knowing whether the commit of a structural CFI specifically changes functionality of the interaction's feature, this casts doubt whether the data changed by commit of a dataflow-based CFI flows into instructions that implement the functionality of the CFI's respective feature \\
\fi

\subsection*{\textbf{RQ3: Discussion of Author Interactions}}\label{sec:disc_author_interactions}

- we find that our dataflow analysis was able to reveal many additional developers that did not directly participate in the development of a feature, but rather contributed commits whose data is used inside features \\
- in most features of \textsc{gzip}, \textsc{bzip2} and \textsc{lrzip}, we detect a similar or higher number of unique df authors compared to the number of structural authors \\
- while most unique df authors are naturally found for features with more dataflow than structural authors, there also exist unique df authors for features with more structural authors \\
- although structural authors often interact with their respective features via outside dataflow, the average of slightly over 50\% across the mentioned projects is too low to say that they almost always do so \\
...
- here we were able to gather initial evidence that the number of authors implementing a feature strongly increases with its size. \\
- in contrast, we found the opposite to be true for the number of authors influencing it through outside dataflow. \\
- however, as only a single project delivered statistically significant results, we consider the question to be completely open to further research. \\
- \textsc{xz} and \textsc{bzip2} are unsuitable for the computation of a significant correlation due to their small number of different authors and \textsc{lrzip} due to its small number of features \\
...
- overall, we can see that the number of respective commits interacting with a feature compared to other features in the same project is strongly connected to the number of respective authors interacting with the feature compared to other features \\
- specifically, the two are positively correlated with a coefficient of 0.73 and a p-value of 0.004 for the number of structural commits and authors and a coefficient of 0.54 and a p-value of 0.058 for the number of outside dataflow commits and authors \\
- thus, the number of commits interacting with a feature can also inform us about the number of authors that interact with it \\
- in RQ1, we discovered that the majority of the structural commits of the four largest features of \textsc{gzip} are identical. \\
- logically, this also applies to x\% of the structural authors of these features. \\
- as discussed in the pervious section, the same applies to the outside commits of the two feature-clusters of \textsc{gzip}, although the proportion of identical commits is lower there. \\
- similarly, the proportion of identical dataflow authors is x\% for the first cluster formed by the four largest features and y\% for the second cluster of smaller features excluding \textsf{recursive} \\
% authors that implemented features of the first cluster also implemented features of the second cluster??
% gzip in total x authors structurally interacting with features and a total of y authors interacting with features through outside dataflow
% this means that only x-y of the authors affecting features through dataflow do not implement features in any way
% here it is important to note that \textsc{gzip} and its used library \textsc{gnulib} have 14 unique contributors, which means that all developers interact with features either structurally or through dataflow or both 
% this is also not surprising given \textsc{gzip}'s high extent of feature-code making up the majority of its instructions 
% that the extent of feature-code plays a role in what share of authors interact with features can also be seen for \textsc{bzip2} with an extent of 8.2\%
% while there are 19 contributors in \textsc{bzip2}'s git-repository, there only exist 5 unique authors interacting with its features in any way
