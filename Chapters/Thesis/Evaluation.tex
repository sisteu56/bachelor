...
- this means that most detected dataflow interactions of features originate outside of its regions and subsequently flows into them \\
...
- during the evaluation of the first part of RQ1, we found that commits that structurally interact with features have an increased probability of influencing other features via outside dataflow \\
- to continue our investigation in this regard, we have determined the proportion of outside commits of features that are part of structural CFIs as well \\
...
- we can therefore confirm that a significant proportion of the data that flows into a feature probably originates from other features or has been modified there. \\

- the regression plots of \textsc{gzip} showed that the results of the linear regressions were mainly caused by two clusters of features \\
...
- in the evaluation of RQ1, we have already explained that, due to feature-nesting, the commits that structurally interact with the features from the first cluster are almost identical \\
- this suggests that the inside commits of said cluster are also fairly congruent and that this could also apply to the outside commits of the two clusters \\

- correlation of gzip mainly driven by two clusters discussed in our evaluation => in eval of RQ1 we also found out that the commits the features of the cluster with large size interact with are largely identical => we found the same to be true for the outside commits of cluster with small sizes and for the outside and inside commits of the other cluster => due to feature-nesting => makes the data perhaps less significant, as the data of many commits could flow into instructions that do not implement functionality of the features the commits interact with
