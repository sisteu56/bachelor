...
On the other hand, we might be dealing with features that occur in the same code space, but implement separate functionality from each other.
...
As our analysis cannot detect whether we are dealing with structurally interacting features or those implementing separate functionality, it does not make sense for us to differentiate between the two cases.
We say that in both cases we are dealing with, at least partially, \emph{nested} features.
If a developer introduces changes to such nested features, the according commit will also produce structural CFIs with all of the respective features.
In the case of structurally interacting features, the commit affects code implementing functionality of both features at the same time.
Thus, the commit technically only deals with a single concern in relation to features, as it does not implement functionality of either feature separately.
In the case of code belonging to multiple features, but only implementing functionality for one one of them, the commit only deals with a single concern as well.
...
In both cases of \emph{feature-nesting}, the produced structural CFIs do not accurately depict the responsibilites of the respective commit.
...
\end{definition}
Feature-related concerns should be used as a measure to gauge the responsibilities of a commit that are difficult to decipher due to feature-nesting. 
...
To accommodate later evaluations using feature-related concerns of a commit and feature nesting factors, we create reports featuring a more complex datatype than a sole structural CFI.
...
To determine a feature's nesting factor~\ref{def:feature_nesting_factor}, we have to additionally compute the instructions where its feature regions do not appear exclusively.
The computation is identical to that of a feature's size with the sole difference being that we only consider interactions with multiple features.
To compute the feature-related concerns of a commit~\ref{def:feature_related_concerns}, we examine the sets of features present in the complex structural CFIs the commit is part of.
For every new set of features that is not identical to any previous set we encountered, the number of concerns is increased by one.  \\
