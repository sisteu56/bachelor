...
In this case, it is impossible to tell which features the commit really changed just from the raw data of its structural interactions.
...
Therefore, we say that the commit only deals with a single feature-related concern as long as the features it interacts with are part of the same code space.
In our analysis, we are working with instructions and therefore speak of the same code space of features when the instructions we are dealing with belong to the same set of feature regions.
...
\begin{lstlisting}[language=C++, caption={Example use-case for the concerns of a commit}, label=DescriptiveLabel]	
...
\end{lstlisting}
\textsf{The above code-snippet illustrates how considering the concerns of a commit is more insightful than simply looking at the features it structurally with. \\ 
