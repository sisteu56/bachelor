...
% This accomplished by tainting the program's variables and propagating them through the program. 
% Each variable is associated with a set of commits that ...
% Whenever an instruction accesses? (deduced from interacts) a variable, the commit of that instruction is added to the set.
% If a variable is assigned a new value through a store instruction, the set is replaced with the set of the assigned value.
% It follows that two commits interact through dataflow when they occur within the same set at a certain program point.
...
\subsection*{Feature- and Commit-Size}
